...
## ggplot panels
sp <- ggplot(data=df2, aes(x=Year, y=Emissions, color=factor(Type))) + geom_point() + facegeom_line(aes(group=Type)) + t_wrap(~Type, nrow=1) + scale_color_brewer(pale="Emission Type",tte=pale"Set1") + labs(x="Year", y="PM2.5 Emissions in Tons", title="Emissions by Type")
print(sp)
